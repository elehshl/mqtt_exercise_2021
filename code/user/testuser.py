...
#Testcar Feedack For Arrival At User And Arrival At Destination
    elif msg[0] == "hshl/mqtt_exercise/"+topictype+str(idCar)+"/call/back" and str(js['msg']) == "arrival": # antwort von dem testcar das ich angeschrieben habe
        destination = ""
        ...
        send(json.dumps(data),"hshl/mqtt_exercise/"+topictype+str(idCar)+"/call/destination") # dem testcar  mein ziel mitteilen
        subtopic.append("hshl/mqtt_exercise/"+topictype+str(idCar)+"/call/destination/back") # hinzufügen der des neuen topics
        receive() # warten auf antwort
    elif msg[0] == "hshl/mqtt_exercise/"+topictype+str(idCar)+"/call/destination/back" and str(js['msg']) == "arrival at destination": #wenn das fahrzeug angekommen ist ??
